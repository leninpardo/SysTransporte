...
class _Permisos extends Main
{
   function index($query , $p ) {
    ...
    function Modulos($p)
    {
		...
		select m.id_modulos,m.descripcion,p.acceder,p.insertar,p.modificar,p.eliminar,c.descripcion,p.id_permiso
FROM
seguridad.permisos as p
right outer join seguridad.perfil as c ON  p .id_perfil = c.id_perfil
right outer join seguridad.modulos as m on p.id_modulos=m.id_modulos and c.id_perfil=:p1 WHERE m.idpadre is null and m.estado=1
");
...
seguridad.permisos as p
right outer join seguridad.perfil as c ON  p .id_perfil = c.id_perfil
right outer join seguridad.modulos as m on p.id_modulos=m.id_modulos and c.id_perfil=:p1 WHERE m.idpadre=:p2 and m.estado=1
                 ");
            $stmt->bindValue(':p1', $p , PDO::PARAM_INT);
    ...
    {
        $stmt = $this->db->prepare("DELETE FROM permiso where id_perfil = :p1");
	...
	{ 
	   $sql =$this->Query("seguridad.sp_permiso(:p1,:p2,:p3,:p4,:p5,:p6,:p7)");
